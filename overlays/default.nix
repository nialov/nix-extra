...
in
{
  ...
  # Build with cmake
  hdf5-full =
    (prev.callPackage "${inputs.lmix-flake-src.outPath}/pkgs/hdf5" {
      inherit (prev) stdenv;
      mpiSupport = true;
      mpi = prev.openmpi;
      fortranSupport = true;
  ...
  pre-commit-hook-ensure-sops = prev.pre-commit-hook-ensure-sops.overridePythonAttrs (prevAttrs: {
  ...
  rstcheck = prev.rstcheck.overrideAttrs (
    _: prevAttrs: {
      propagatedBuildInputs = prevAttrs.propagatedBuildInputs ++ [ prev.python3Packages.sphinx ];
    }
  );
  ...
  gdal-mdb = prev.gdal.overrideAttrs (
    _: prevAttrs: {
      buildInputs = prevAttrs.buildInputs ++ [
        prev.mdbtools-unixodbc
        prev.unixODBC
      ];
    }
  );
  ...
  haskellPackages = prev.haskellPackages.override {
    overrides = self: super: {
      easyplot = null;
      huzzy = super.callPackage ./packages/tasklite/huzzy.nix { inherit inputs; };
      iso8601-duration = super.iso8601-duration.overrideAttrs (
        _: _: {
          meta.broken = false;
          postPatch = ''
      ...
      simple-sql-parser = super.simple-sql-parser.overrideAttrs (
        _: _: {
          meta.broken = false;
          postPatch = ''
  ...
  tasklite-core = final.haskell.lib.justStaticExecutables final.haskellPackages.tasklite-core;
